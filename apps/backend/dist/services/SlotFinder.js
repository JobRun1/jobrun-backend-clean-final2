...
class SlotFinder {
     ...
     */
    static async findEarliestSlot(request) {
     ...
     */
    static async findAvailableSlots(request) {
        ...
        while (currentDate <= endDate) {
            ...
            for (const rule of dayAvailability) {
                const slots = this.generateSlotsInWindow(currentDate, rule.startTime, rule.endTime, durationMinutes, timeWindow);
                for (const slot of slots) {
                    if (!this.isBlocked(slot.start, slot.end, blockedTimes) &&
     ...
     */
    static generateSlotsInWindow(date, startTime, endTime, durationMinutes, timeWindow) {
        const slots = [];
     ...
     */
    static isBlocked(slotStart, slotEnd, blockedTimes) {
        for (const block of blockedTimes) {
            ...
            if (blockDate.getTime() !== slotDate.getTime())
                continue;
     ...
     */
    static isBooked(start, end, bookings) {
     ...
     */
    static isInTimeWindow(time, window) {
     ...
     */
    static async isDayClosed(clientId, date) {
     ...
     * Find next open day
     */
    static async findNextOpenDay(clientId, afterDate) {
