...
class NotificationService {
     ...
     */
    static async sendHandoverNotification(clientId, payload) {
        const result = {
            smsSent: false,
            emailSent: false,
            errors: [],
     ...
     * Get client notification settings
     */
    static async getClientNotificationSettings(clientId) {
     ...
     */
    static async sendSMS(phoneNumber, payload) {
    ...
    static formatSMSMessage(payload) {
        const { customerName, customerPhone, urgencyLevel, reason, dashboardLink, } = payload;
...
${dashboardLink}`;
     ...
     */
    static formatEmailSubject(payload) {
     ...
     */
    static formatEmailBody(payload) {
        const { conversationId, customerName, customerPhone, lastMessages, urgencyScore, urgencyLevel, reason, triggers, dashboardLink, } = payload;
...
`;
     ...
     */
    static buildDashboardLink(clientId, conversationId) {
     ...
     */
    static getLastMessages(conversationId, count = 10) {
        // Use ConversationMemory to get recent messages
        return ConversationMemory_1.ConversationMemory.getLastNMessages(conversationId, count);
    }
}
