...
class Safety {
     ...
     */
    async checkOutput(output, context, agentName) {
        const violations = [];
        const warnings = [];
        // Check 1: Confidence threshold
        if (output.confidence < 0.55) {
            violations.push(`Confidence too low (${output.confidence}). Must be >= 0.55 to execute actions.`);
        }
     ...
     */
    checkAction(action, context, agentName) {
        const violations = [];
        const warnings = [];
        switch (action.type) {
            case 'CREATE_BOOKING':
            case 'UPDATE_BOOKING':
                ...
                if (!this.hasCustomerConfirmation(context, action)) {
                    violations.push(`${action.type} requires explicit customer confirmation. Not detected in context.`);
                }
     ...
     */
    hasCustomerConfirmation(context, action) {
        if (!context.input.message) {
            return false;
        }
