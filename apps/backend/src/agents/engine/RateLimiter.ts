...
interface CustomerRateLimitKey {
  agentName: string;
  clientId: string;
  customerId: string;
}
...
export class RateLimiter {
  private executionCounts: Map<string, RateLimitEntry> = new Map();
  private customerMessageCounts: Map<string, RateLimitEntry> = new Map();

  // Global safety limits
  private readonly GLOBAL_MAX_PER_HOUR = 100;
  private readonly GLOBAL_MAX_PER_DAY = 500;

  // Customer-specific limits for proactive messages
  private readonly CUSTOMER_MAX_PROACTIVE_PER_6H = 1;
  private readonly CUSTOMER_REVIEW_REQUEST_COOLDOWN_HOURS = 168; // 7 days
  private readonly CUSTOMER_PAYMENT_REMINDER_COOLDOWN_HOURS = 72;
  ...
  async checkRateLimit(
    ...
    if (customerId && this.isProactiveAgent(agentName)) {
      const customerCheck = this.checkCustomerLimits(
        agentName,
        clientId,
        customerId
      );
   ...
   */
  private checkGlobalLimits(key: string): RateLimitCheck {
    const entry = this.executionCounts.get(key);
    if (!entry) {
      return { allowed: true };
    }
  ...
  private checkAgentLimits(
    key: string,
    limits: NonNullable<AgentConfig['rateLimits']>
  ): RateLimitCheck {
    const entry = this.executionCounts.get(key);
    if (!entry) {
      return { allowed: true };
   ...
   */
  private isProactiveAgent(agentName: string): boolean {
