...
async function testAgentSystem() {
  ...
  for (const testCase of testCases) {
    ...
    const hasExpected = matchedAgents.some(
      (a) => a.getName() === testCase.expectedAgent
    );
  ...
  const manifest = getAgentManifest(orchestrator);
  ...
  for (const agent of manifest.sort((a, b) => {
    ...
    console.log(
      ...
      agent.model.padEnd(12),
      agent.priority.toString().padEnd(10),
